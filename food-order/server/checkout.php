...
if(isset($_POST['submit'])){
   ...
   if($check_cart->rowCount() > 0){
      ...
      if($address == ''){
         ...
         $conn = connectdb();
         $insert_order = $conn->prepare("INSERT INTO `order_detail`(id_user, name, number, email, method, address, total_products, total_price) VALUES(?,?,?,?,?,?,?,?)");
         $insert_order->execute([$id_user, $name, $number, $email, $method, $address, $total_products, $total_price]);

         $conn = connectdb();
         $delete_cart = $conn->prepare("DELETE FROM `cart` WHERE id_user = ?");
         $delete_cart->execute([$id_user]);
...
<form action="" method="post">
   ...
   <input type="hidden" name="name" value="<?= $fetch_profile['fullname'] ?>">
   ...
   <div class="user-info">
      <h3>your info</h3>
      <p><i class="fas fa-user"></i><span><?= $fetch_profile['fullname'] ?></span></p>
